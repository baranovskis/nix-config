...
```bash
just --justfile /etc/nixos/Justfile --working-directory /etc/nixos
```

You can also use `just` directly when inside the `/etc/nixos` directory.
...
- `erebor/`: Desktop machine configuration (The Lonely Mountain)
  - `default.nix`: Host-specific settings (hostname, boot, kernel, NVIDIA, VFIO, restic backups, module enables)
...
- Kernel selection (e.g., linuxPackages_zen)
- Hardware-specific kernel parameters (ACPI overrides, VFIO passthrough IDs, etc.)
- GPU driver configuration (NVIDIA/AMD packages, modesetting)
- VFIO/GPU passthrough (IOMMU, kernel modules, udev rules, Looking Glass)
- Restic backup configuration
- Module enable flags (which opt-in modules to activate)
...
- `default.nix`: Main module entry point — imports core, wm, profiles, and all opt-in modules

**Core (`modules/core/`)** — Always-on base system:
...
**Window Managers (`modules/wm/`)** — Swappable desktop environments:
- `gnome.nix`: GNOME desktop + GDM + Stylix theming (`modules.wm.gnome.enable`)

**Desktop Profile (`modules/profiles/desktop/`)** — Desktop peripherals and services:
- `audio.nix`: PipeWire audio
- `bluetooth.nix`: Bluetooth
- `flatpak.nix`: Flatpak daemon + XDG portal (packages managed via home-manager)
- `gnupg.nix`: GnuPG
- `nuphy.nix`: NuPhy keyboard support
- `printing.nix`: CUPS printing
- `solaar.nix`: Logitech Solaar device management
- `zen-browser.nix`: Zen Browser (declarative wrapFirefox)

**Opt-in Modules (`modules/*.nix`)** — Enabled per-host via `modules.<name>.enable`:
- `ai.nix`: Ollama with CUDA acceleration
- `btrfs.nix`: Btrfs filesystem support with auto-scrub
- `docker.nix`: Docker containerization
- `gaming.nix`: Steam, Proton-GE, GameMode, MangoHud
- `gpu.nix`: GPU configuration (OpenGL/Vulkan)
- `nfs.nix`: NFS server/client
- `rdp.nix`: Remote desktop (XRDP + GNOME Remote Desktop)
- `sunshine.nix`: Sunshine game streaming
- `virtualization.nix`: KVM/QEMU/libvirt
- `zfs.nix`: ZFS filesystem support with auto-scrub and snapshots
...
### Shared Library (`lib/`)
- `default.nix`: `mkHost` and `mkHome` helper functions for flake outputs
- `stylix.nix`: Shared Stylix theme settings (fonts, icons, cursor) imported by both NixOS and home-manager

### Home Manager Configuration (`home-manager/`)
Shared user configuration across all hosts:
- `default.nix`: Main home-manager configuration entry point (loads Stylix + nix-flatpak HM modules)
- `config/`: Theming and appearance
  - `default.nix`: Config module imports
  - `stylix.nix`: Stylix theme settings (imports shared config from `lib/stylix.nix`, adds wallpaper + GTK Flatpak support)
- `programs/`: User application configurations (shared across all hosts)
  ...
  - `flatpak.nix`: Declarative Flatpak packages, remotes, and overrides (via nix-flatpak homeManagerModules)
...
### Assets (`assets/`)
- `wallpapers/`: System wallpapers (used by Stylix for theming and as desktop background)
- `screenshot.jpg`: Desktop screenshot for README
...
- Color scheme is auto-generated from the wallpaper image (no external scheme file)
- Shared theme settings in `lib/stylix.nix` (fonts, icons, cursor)
- System-level Stylix in `modules/wm/gnome.nix` (GDM/system themes)
- User-level Stylix in `home-manager/config/stylix.nix` (GTK Flatpak support)
- Dark polarity, Inter font, JetBrains Mono for terminal, Papirus icons, Capitaine cursors
...
- `stylix`: System-wide theming (NixOS + home-manager modules)
- `solaar`: Logitech device management (NixOS module)
- `zen-browser`: Zen Browser (declarative wrapFirefox wrapper)
- `nix-flatpak`: Declarative Flatpak management (via homeManagerModules)
...
- **Modularization strategy**:
  ...
  - Use `modules/` for shared system configuration
